...
class SimulatedAGVNode(Node):
    def __init__(self, agv_id):
        ...
        self.charging_stations = [(0, 0), (9, 9)]
    ...
    def execute_next_task_step_timer_callback(self):
        ...
        current_path_state = self.path_states[self.path_index]
        ...
        if self.state == AGVTaskState.CHARGING and self.path_index >= len(self.path_states):
            self.battery_level = 100.0
            self.state = AGVTaskState.STANDBY
...
def main(args=None):
    ...
    agv_node = SimulatedAGVNode(agv_id="agv_02")
    try:
        rclpy.spin(agv_node)
